    ...
    const Matrix4& view_matrix, const Matrix4& proj_matrix)
{
    ...
    */
    ...
    Matrix4 BSRAS = skeleton_.BoneSpaceToRotAxesSpace(bone_name);
    Matrix4 jointRotate = pose_.JointRotation(bone_name);
    Matrix4 RASBS = skeleton_.RotAxesSpaceToBoneSpace(bone_name);


    Matrix4 ctm = parent_transform * BSRAS * jointRotate * RASBS;


    Vector3 boneDirLen = skeleton_.BoneDirectionAndLength(bone_name);

    quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0, 0, 0), Point3(0, 0, 0), Point3(0, 0, 0) + boneDirLen, 0.01);
    ...
    if (bone_name == "lowerback") {
        Matrix4 lowerBackTransform = Matrix4::Translation(Vector3(0, -0.2, 0)) * Matrix4::RotationX(GfxMath::ToRadians(30)) * Matrix4::Scale(Vector3(0.13, 0.25, 0.13));
        quick_shapes_.DrawSphere(ctm * lowerBackTransform, view_matrix, proj_matrix, Color(0, 0, 0));
    }
    if (bone_name == "upperback") {
        Matrix4 upperBackTransform = Matrix4::Translation(Vector3(0, -0.05, 0)) * Matrix4::RotationX(GfxMath::ToRadians(180)) * Matrix4::Scale(Vector3(0.13, 0.2, 0.13));
        quick_shapes_.DrawSphere(ctm * upperBackTransform, view_matrix, proj_matrix, Color(1, 1, 0));
    }
    if (bone_name == "thorax") {
        Matrix4 thoraxTransform = Matrix4::Translation(Vector3(0, 0.05, 0)) * Matrix4::Scale(Vector3(0.1, 0.13, 0.1));
        quick_shapes_.DrawSphere(ctm * thoraxTransform, view_matrix, proj_matrix, Color(0, 0, 0));
    }
    if (bone_name == "lowerneck" || bone_name == "upperneck") {
    }
    if (bone_name == "head") {
        Matrix4 headSpace = Matrix4::RotationX(GfxMath::ToRadians(-20)) * Matrix4::Scale(Vector3(0.11, 0.14, 0.11));
        Matrix4 headTransform = headSpace * Matrix4::Translation(Vector3(0, 0, 0.5)) * Matrix4::Scale(Vector3(1, 1, 1));
        quick_shapes_.DrawSphere(ctm * headTransform, view_matrix, proj_matrix, Color(1, 1, 0));

        Matrix4 lEye = headSpace * Matrix4::Translation(Vector3(-0.3, 0.35, 1.25)) * Matrix4::RotationX(GfxMath::ToRadians(30)) * Matrix4::Scale(Vector3(0.2, 0.2, 0.2));
        Matrix4 rEye = headSpace * Matrix4::Translation(Vector3(0.3, 0.35, 1.25)) * Matrix4::RotationX(GfxMath::ToRadians(30)) * Matrix4::Scale(Vector3(0.2, 0.2, 0.2));
        ...
        Matrix4 lAntenna = headSpace * Matrix4::Translation(Vector3(-0.5, 0.6, 0.5)) * Matrix4::RotationZ(GfxMath::ToRadians(5)) * Matrix4::Scale(Vector3(0.23, 0.77, 0.23));
        Matrix4 rAntenna = headSpace * Matrix4::Translation(Vector3(0.5, 0.6, 0.5)) * Matrix4::RotationZ(GfxMath::ToRadians(-5)) * Matrix4::Scale(Vector3(0.23, 0.77, 0.23));
        Matrix4 lAntenna2 = lAntenna * Matrix4::Translation(Vector3(1.5, 1.5, 1.5)) * Matrix4::Scale(Vector3(1.6, 0.376, 1.6));
        Matrix4 rAntenna2 = rAntenna * Matrix4::Translation(Vector3(1.5, 1.5, 1.5)) * Matrix4::Scale(Vector3(1.6, 0.376, 1.6));
        quick_shapes_.DrawLineSegment(ctm * lAntenna, view_matrix, proj_matrix, Color(0, 0, 0), Point3(0, 0, 0), Point3(1.5, 1.5, 1.5), 0.3);
        quick_shapes_.DrawLineSegment(ctm * rAntenna, view_matrix, proj_matrix, Color(0, 0, 0), Point3(0, 0, 0), Point3(1.5, 1.5, 1.5), 0.3);
        quick_shapes_.DrawSphere(ctm * lAntenna2, view_matrix, proj_matrix, Color(0, 0, 0));
        quick_shapes_.DrawSphere(ctm * rAntenna2, view_matrix, proj_matrix, Color(0, 0, 0));
    }
    ...
    **/
    ...
    Matrix4 child_root_transform = ctm * Matrix4::Translation(boneDirLen);
