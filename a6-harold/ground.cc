                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d.front(), &start);
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d.back(), &end);

    //Create the plane normal by crossing the start to end vector with a vector pointing up
    Vector3 planeNormal = (start - end).Cross(Vector3(0, 1, 0)).ToUnit();

    //if the plane normal is facing away from the camera, turn it around to face the camera
    if (planeNormal.Dot(look) > 0) {
    ...
    //This is the same concept as screenPtHitsSky where a 2d point was projected to 3d
    //The same thing is happening here except instead of just 1 point, we have a vector of points
    //I have modeled this based on screenPtHitsSky due to the similarity and am trying to keep them as close as possible to one another
    //to emphasize the similarity
    std::vector<Point3> projection3d;
    float t;
    Point3 p;

    //loop through all the points in stroke2d and project into 3d
    for (int i = 0; i < stroke2d.size(); i++) {
        ...
        //IntersectPlane instead of IntersectSphere like in screenPtHitsSky since we are working with a plane rather than a sphere
        eyeThroughMouse.IntersectPlane(start, planeNormal, &t, &p);
        //Put the point into the projection vector
    ...
    for (int i=0; i<ground_mesh_.num_vertices(); i++) {
        ...
        float d = P.DistanceToPlane(start, planeNormal);
        float h = hfunc(planeNormal, projection3d, P.ClosestPointOnPlane(start, planeNormal));
        float Py = P.y();

        if (h != 0) {
            float wd = 1 - pow((d / 5.0), 2);
            wd = wd < 0 ? 0 : wd;
            Py = (1 - wd) * Py + wd * h;
            P[1] = Py; //P[1] is P.y()
        }
